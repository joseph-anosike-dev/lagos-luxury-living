...
function HomePage() {
  ...
  const [finder, setFinder] = useState({ status: "Buy", city: "All locations", type: "All types", budget: "Any budget" });
  const [inquirySent, setInquirySent] = useState(false);

  const filteredProperties = useMemo(() => {
    return properties.filter((property) => {
      if (activeFilter !== "All" && property.status !== activeFilter && property.city !== activeFilter) return false;
      if (finder.status !== "Buy" && finder.status !== property.status) return false;
